                    ...
                    <tr key={m.id} className="hover:bg-surface-soft transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider inline-flex items-center gap-1 ${
                          ...
                          m.tier === 'associate_member' || m.tier === 'associate_silver' ? 'bg-slate-100 text-slate-800 border border-slate-300' :
                          m.tier === 'supporting_member' || m.tier === 'founding_platinum' ? 'bg-blue-100 text-blue-900 border border-blue-300' :
                          m.tier === 'patron_member' || m.tier === 'patron_gold' ? 'bg-amber-100 text-amber-900 border border-amber-300' :
                          m.tier === 'benefactor_member' || m.tier === 'benefactor_diamond' ? 'bg-purple-100 text-purple-900 border border-purple-300' :
                          'bg-slate-100 text-slate-800 border border-slate-300'
                        }`}>
                          <Crown className="w-3 h-3" />
                          {m.tierName || m.tier.replace('_', ' ')}
                        </span>
