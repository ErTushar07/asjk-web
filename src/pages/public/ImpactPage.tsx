    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-16">
      <div className="text-center max-w-3xl mx-auto space-y-4">
        ...
        <p className="text-content-secondary text-sm sm:text-base leading-relaxed">
          {t('impact.subtitle', 'We measure our success not simply in funds raised, but in verifiable metrics: liters of clean water pumped, classrooms digitalized, patients treated, and families sheltered.')}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-6 lg:gap-8">
        {impactMetrics.map((m) => {
          ...
          return (
            <div
              key={m.id}
              className="bg-white rounded-2xl sm:rounded-3xl border border-content-border p-3.5 sm:p-8 shadow-brand-sm hover:shadow-brand-md transition-all space-y-3 sm:space-y-4 flex flex-col justify-between min-w-0"
            >
              <div className="space-y-2 sm:space-y-3">
                <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-surface-soft flex items-center justify-center text-brand-purple">
                ...
                <h4 className="font-extrabold text-xs sm:text-base text-content-primary truncate">{t(m.label, m.label)}</h4>
                <p className="text-[10px] sm:text-xs text-content-secondary leading-relaxed line-clamp-3">{t(m.label + '.desc', m.description)}</p>
              </div>
      ...
      <div className="bg-surface-card rounded-3xl p-8 sm:p-12 border border-content-border space-y-6">
        <div className="max-w-2xl space-y-2">
          ...
          <p className="text-xs text-content-secondary">
            {t('impact.sdg_desc', 'Our programs in Jammu & Kashmir directly support global targets for clean water, zero hunger, good health, and quality education.')}
          </p>
        </div>
